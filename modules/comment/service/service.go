...
func (s *service) ListComment(ctx context.Context, req *pb.ListCommentRequest) (*pb.ListCommentResponse, error) {
	...
	comments_slice := make([]*pb.CommentInfo, 0, len(comments))
	for _, comment := range comments {
		comments_slice = append(comments_slice, comment.ToProto())
	}
	return &pb.ListCommentResponse{Comments: comments_slice}, nil
}
...
func (s *service) CreateComment(ctx context.Context, req *pb.CreateCommentRequest) (*pb.CreateCommentResponse, error) {
	...
	comment_id, err := s.commentDAO.Create(ctx, &comment)
	if err != nil {
		return nil, err
	}
	return &pb.CreateCommentResponse{Id: comment_id.String()}, nil
}
